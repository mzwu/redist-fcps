...
# 
#     # get school name
#     school_name <- ffx_es$OBJECTID[idx_in_schools]
# 
#     # calculate the proportion of total population that this school would theoretically have
#     target_ratio <- ffx_elem$prop_capacity[ffx_elem$object_id_school == school_name][1L]
...
#   )
# 
# plans <- redist_smc(map, nsims = 5000,
#                     #constr = constr
#                     )
...
plans <- redist_smc(
  ...
  ms_params = list(ms_frequency = 1L, ms_moves_multiplier = 111),
...
write_rds(plans, here("data-raw/elem25/plans_es_noconstr.rds"), compress = "gz")
